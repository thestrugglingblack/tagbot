...
class TagBot(commands.Cog):
    ...
    async def add(self, ctx):
        ...
        # Split the message into parts for more precise parsing
        parts = ctx.message.content.strip().split()

        # Expected format: "tagbot add <platform> <tag>"
        if len(parts) < 3:
            await self._send_error(ctx, "Usage: `tagbot add <platform> <tag>`")
            return

        platform = parts[2].lower()  # Get the platform (3rd word)
        tag = " ".join(parts[3:])  # Join the rest as the tag (supports multi-word tags)
